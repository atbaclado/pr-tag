		...
		<div class="row">
			<!-- Sidebar -->
		 	<aside id="activerSide" class="col-md-4 sidebar sidebar-left" style="height:450px;">
				...
				<div class="row widget">
					<div  class="col-xs-12">
						<h4 class="btn" id="activer"><a href="completed.php">Completed PRs</a></h4>
						<p>PRs that were completed</p>
					</div>
				</div>
				<div class="row widget">
					<div id="activerRow" class="col-xs-12">
						<h4 class="btn" id="activer">Draft PRs</h4>
			...
			<article class="col-md-8 maincontent">
				...
				<div class=" col-md-9 col-lg-9 "> 
					...
					<tbody>
						<form action="php/gen_draft.php" method="get">
							<?php
								$query = "SELECT * FROM purchase WHERE pr_office = " . $_SESSION['user_ofc'] . " AND NOT EXISTS (SELECT * FROM tracking WHERE pr_id = track_prid)";
								
								$result = pg_query($dbconn,$query);
								if(!$result) {
									$errormessage = pg_last_error();
									$_SESSION['errormsg'] = 'PR draft. ' . $errormessage;
								...
								if($rs == 0) {
									...
									for($ri = 0; $ri < $rs; $ri++) {
										...
										<td><a href="#">'. $row['pr_id'] .' </a></td>
										<td>'. $row['pr_date'] .'</td>
										<td>'. $row['pr_purpose'] .'</td>
										<input class="hidden" type="number" value="'.$row['pr_id'].'" name="id[]" />
										<td><input class="btn btn-default" type="submit" value="'.$ri.'"   name="viewer" /></td></tr>';	
									}
								}
							?>
						</form>
